    ...
    for name in actions[0]:
        ...
        if name in rotation_fields and len(actions) == 1:
            ...
            eigenvalues, eigenvectors = np.linalg.eigh(
                np.einsum("i,ij,ik->jk", weights, quats, quats)
            )
...
def interpolate_actions(
    ...
    for name in current:
        if name in rotation_fields:
            ...
            output[name] = (
                (1 - alpha) * np.asarray(previous[name]) + alpha * np.asarray(current[name])
            ).tolist()
    return output
...
class ExecutionSlot:
    ...
    def resolve(
        ...
        *,
        ...
        weights = np.exp(
            [-coefficient * (newest - c.received_at_s) * control_hz for c in contributions]
        )
        weights /= weights.sum()
        output = blend_actions([c.action for c in contributions], weights, rotation_fields)
        provenance = {
            "contributions": [c.provenance(w) for c, w in zip(contributions, weights)],
            "transition": None,
        }
        if self.anchor is not None:
            output = interpolate_actions(
                self.anchor.action, output, self.anchor.alpha, rotation_fields
            )
            provenance["transition"] = {
                "alpha": self.anchor.alpha,
                "deadline_s": self.anchor.deadline_s
                if math.isfinite(self.anchor.deadline_s)
                else None,
